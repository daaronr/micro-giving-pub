## Second problem set with suggested answers: Preferences, Utility, Consumer optimization {-}
...
Why? Ordinal utility functions are only identified up to a monotonic transformation. So doubling a utility function leads to the same choices
\

FALSE: 

(a): False because we cannot compare ordinal utility across individuals (because it's only identified up to a monotonic transformation)

(b): False because Utility functions can be stated in terms of as many goods, services, and activities as you like. (Indifference curves are hard to depict in more than 2-3 goods, however) 

(d): False: It's true that demand function is homogenous of degree zero; however, here we did \textit{not} multiply prices and income by the same amount -- we only changed income

(e) False: While we can identify some elements of preferences by observing an individual's choices
...
[I made a video on some related questions here](https://www.youtube.com/watch?v=Qm3HfqkyqAo&feature=youtu.be)
...
For optimisation (where the second order condition holds, and where we consume some of both so it's not a corner solution; this will be the case here, points may be awarded here if anyone mentions this and more if they derive it):

$MU_{\ell}/MU_{c}=MRS=p_{\ell}/p_{c}$

$\rightarrow{\frac{2}{3}}c^{\frac{1}{3}}\ell^{-\frac{1}{3}}/{\frac{1}{3}}c^{-\frac{2}{3}}\ell^{\frac{2}{3}}=\frac{2}{1}$
...
$2\ell+ \ell=50\rightarrow3\ell=50\rightarrow \ell =50/3}$

$c=\ell=50/3$

(Side note, you might argue leeks are 'discrete' and not continuous)^[So you could solve this for the optimal 'whole number' of leeks and chocolates... students get credit for this on an exam, but it's not necessary.]
...
Set it equal to the new price ratio: $2c/\ell=1 \rightarrow c=\ell/2}$

New budget constraint: $2\ell+2c=50$

Substituting in: 

$2\ell+\ell=50\rightarrow\ell=50/3 \rightarrow c=\frac{50}{6}$
