...
Formally  (for 2 goods), the indifference curve is th set of pairs of ${X,Y}$ such that $U(X,Y)=c$ for some constant c.^[Recall the "level sets" from the maths revision.]
...
`r format_with_col("Note: I've asked this as a MCQ on exams!","blue")`
```
...
Graphically (i.e., what point on the graph depicts this, considering 'other goods' are considered to cost \$1 per other good), how much \$ would have been enough to compensate for abstaining?^[`r format_with_col("Adv: Hard question, foreshadowing future concepts like the inefficiency of in-kind subsidies... How much would Willie need to be paid to get him to U_3 *without* the restriction?  Is this more or less than $5k?","RawSienna")`]
...
`r format_with_col("Warning:","red")` this min function seems backwards, but it's correct. I've answered this question many times in office hours and on the forum.  No one believes me at first but in the end they realize that this is the case.
...
OK, I'll attempt to **explain the min function**

Perhaps it's easier to understand as the equivalent

$$U(Wheels,Frames)=\min(\frac{Wheels}{2},Frames)$$

In maximising this 'min' function subject to a budget constraint (or in minimising expenditure while attaining a certain utility) you will always set the elements *within* the $min()$ operator to be equal to one another. To do otherwise would squander resources. ... E.g., if utility is $min(x,y)$ and I set $x=1$ and $y=3$ then $\min(1,3)=1$, so I've wasted expenditure on the 'y' good. 

\

So in the above 'bicycle' case I'll  always set $\frac{Wheels}{2} = Frames$, i.e., I'll always buy *twice* as many wheels as frames. 
...
*Shortcut to determining the optimal choice, with perfect complements*: figure out the proportions it will be consumed in
