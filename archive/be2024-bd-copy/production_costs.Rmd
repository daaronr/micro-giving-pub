...
- When the firm chooses inputs optimally (and where it uses both), it chooses K and L to set the marginal rate of technical substitution $RTS(K,L)$ equal to the price ratio of these inputs ($w/r$... wages $w$ per unit of for labor, rent $r$ per unit of capital).
...
(We discuss this further [below](#irsdrsreasons).)
...
### Arguments/reasons for scale (dis)economies {-#irsdrsreasons}
...
`r format_with_col("Mock exam question: Give one reason why production may exhibit 'increasing returns to scale' and one reason why it may have 'decreasing returns to scale'.", "gray")`
