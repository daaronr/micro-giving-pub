...
`r format_with_col("Note:","red")` The module moves through select parts of textbook; coverage and order given in handout.
...
```{r  fig.cap = '', out.with='70%', fig.asp=.4, fig.align='center',  echo = FALSE}
...
`r format_with_col("This last comment is especially relevant for non-native speakers of English.","gray")`
...
```{r  fig.cap = '', out.height='2.3in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
Do US farm subsidies help or hurt Africans in net? `r format_with_col("Think about: what do we mean 'in net'?","RawSienna")`
...
```{r  fig.cap = '', out.height='2.5in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='2.2in', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics("contourlines_example.png")
```
...
```{r  fig.cap = '', out.height='2.2in', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics("simeqn.png")
```
...
`r format_with_col("This will be probably skipped in lecture, but please read it. This will be referred to in problem sets/tutorials.","brown")`
...
`r format_with_col("Also 'forbidden' in the classical model: the *time* or *frame* in which I make the *decisions* must not affect my choices. This will be relaxed when we discuss behavioral economics.","gray")`
...
`r format_with_col("($\ast$technically, we also require something called 'continuity', which also yields continuous utility and demand curves that don't have jumps,  but we won't discuss this).","gray")`
...
`r format_with_col("Note: I've asked this as a MCQ on exams!","blue}{Comprehension Q: Rank order of preference between points A-E.} \textcolor{gray")` (Answers to most comprehension questions are given in footnotes. Try to answer it yourself before looking at the footnote.) ^[Ans: $E \succ A \sim B \sim C \sim D \succ F$]
...
Details:  `r format_with_col("Sign in with email, orthogonal employability treatments.","gray}{2 rounds over 2 academic years}. \textcolor{gray}{2 prizes in round 1, Amazon  only in round 2.}  \textcolor{gray}{First covering Faculty of Social Science, then all departments.} \textcolor{gray")`
