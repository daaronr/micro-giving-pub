...
<aside class="notes"><small>Economists and decision theorists have tried to come up with and defend such rules.  They have started from these 'reasonable axioms' and followed their logical implications for  individual choices, individual responses to changes in prices and income,
market prices and quantities and their responses,
 `welfare' and inequality outcomes for entire economies, etc.</small></aside>
...
<aside class="notes">
This is the 'content' of the model; otherwise it seems circular (we max util, what's util, the thing we max). Content: we consider tradeoffs among *all* alternatives in a consistent way.
</aside> 
...
<aside class="notes">Adv: In general, measuring/comparing utility  is a difficult but well-studied issue as it is important for methodology and welfare analysis.
...
- Similar transitivity for indifference ($\sim$)

  - Note: indifference is also 'reflexive' so $A \sim B \rightarrow B \sim A$
...
<center> <small> (similar to nonsatiation, 'monotonicity') </small> </center>

```{r  fig.cap = '', out.width='65%', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics("picsfigs/moreisbetter.png")
```

<center> *Draw this!* </center>

<small> `r format_with_col("If the product is a 'bad' (e.g., pollution), redefine as the *absence* of the product","gray")` </small>
...
'Rate at which you're willing to forgo consuming $Y$
to consume one more $X$'
...
```{r  fig.cap = 'Budget constraint for two goods, slope $-P_x/P_y$', out.width='75%', fig.asp=.4, fig.align='center',  echo = FALSE}
