...
\pdfnote{is similar to our text, but with more maths and formalism, and some extensions (but fewer applications)}
...
\pdfnote{Reasons to model optimising behavior if it is not predictive.
...
\pdfnote{This is a difficult concept}
...
\pdfnote{Continuity: If bundle A pref B and $C$ 'lies within an epsilon radius of $B$ than A pref C. \textCR
...
\pdfnote{this is the impact of price on the 'compensated' or 'Hicksian' demand. \textCR
...
\includegraphics[height=3in]{picsfigs/axioms.png}

\pdfnote{LC: Keep these up on the board}
...
\pdfnote{Find by solving for utility-max choices, computing utility}
...
\textcolor{olive}{MOVE to Autor's notes/pptx here}

***

\textcolor{gray}{... doctoral prep}
...
\textcolor{gray}{Note: These functions are actually used in 'structural' estimates of demand used in merger analysis, pricing, litigation consulting, etc.}
...
\pdfnote{Roy's identity: involved proof, uses envelope theorem. \textCR
 Intuition... utility decreases when  $p_X$ rises ...  $\frac{\partial V}{\partial p_x}$.
From  initial consumption point we lose  $d_X \times \epsilon$ in spending power. \textCR
...
\pdfnote{A similar result applies to a change in *another* good's price}
...
\textcolor{gray}{(See additional notes in handout; we will discuss further)}
...
\pdfnote{We mentioned this before (Autor notes)}

\pdfnote{The choice axioms imply this matrix must have certain properties, such as 'Negative semi-definiteness' \textCR
...
\includegraphics[height=3in]{picsfigs/elasticities_are_logs_post.png}
...
\textcolor{red}{Challenge question:} Why might the area under the estimated *market* demand curve mis-state the true consumer surplus? What are the implications of this?
...
\pdfnote{More general: 'Production possibility set' \textCR
Set of all vectors (called 'production plans') that contain feasible combinations of inputs and outputs \textCR
 Allows us to conside things like economies of scope }
...
\includegraphics[height=4.5in]{picsfigs/prod_isoquant2.png}

\pdfnote{Q: How do these isoquants display 'diminishing marginal product' of each input?}

\pdfnote{Ans: Slope gets steeper as we move towards more capital, less labour \textCR
Implying with more K we need to add more and more K to make up for a loss of a small amount of L. \textCR
 Slope shallower with more L, less K ... similar point}

\pdfnote{'Iso' probably means 'the same' in some language. \textCR
So 'isoquants' connect all points, all combinations of output that produce the same 'quant'(ity.)}

\pdfnote{The slope of these is called the 'Rate of Technical Substitution,' to be defined shortly}
...
\includegraphics[height=4.5in]{picsfigs/prod_isoquant3.png}
...
\pdfnote{This is fundamentally similar to the reason why the MRS was the ratio of marginal utilities!}
...
\pdfnote{If so, what limits the size of firms? I.e., why don't we see only one firm in each industry?}
...
- Managerial issues and coordination problems, bigger 'centre' to lobby for favours.  \textcolor{gray}{See 'theories of the firm'}
- Harder to give incentives to top manager/CEO?

\pdfnote{Adv: I think the first point is valid, \textCR
 but it doesn't suggest that 2 firms would be more efficient than 1; \textCR
these DRS would set in as a function of  *total* output, not a single firm's output. \textCR
 The second and third arguments are complicated and perhaps harder to justify. \textCR
 The case for DRS is unproven, IMHO}
...
\pdfnote{Rem: in a perfectly competitive market, firms must produce as cheaply as all other firms, or exit}
...
\pdfnote{This is not covered at this point in the text. \textCR
...
\textcolor{gray}{Contrast to *opportunity costs*.}
...
\pdfnote{Please *don't* use accounting costs to make a decision going forward. }
-->


***

\includegraphics[height=4.5in]{picsfigs/firstclass.png}
...
\pdfnote{Economic profits differ from accounting profits, \textCR
 in part because economists charge the 'cost of capital', or the alternate use of the machine, against your profit. \textCR
  If I am investing \pounds1000 and I earn \pounds20 per year, but I could otherwise earn \pounds10 per year from a similarly risky investment, \textCR
  my *economic profit* is only \pounds10 per year}
...
\pdfnote{Again, this is very much like the consumer case. Recall the consumer set the MRS equal to the price ratio \textCR
...
\includegraphics[height=1.8in]{picsfigs/costmin2.png}

***

\includegraphics[height=4.5in]{picsfigs/costmin1.png}

\note{What point on this curve will minimize cost? \textCR
The one on the lowest 'isocost line' ... similar budget line for consumers}

***

\includegraphics[height=4.5in]{picsfigs/costmin2.png}

\pdfnote{All points on this line TC1 cost the same amount \textCR
This holds  $= TC = wL + vK$ constant. \textCR
The slope is the price ratio $w/v$ \textCR
The point on this line that produces the most is the one *tangent* to an isoquant}


***

\includegraphics[height=4.5in]{picsfigs/costmin3.png}

\pdfnote{The same $q$ can be produced by combinations other than $K^*, L*$, but these will *cost more*. \textCR
...
\includegraphics[height=2in]{picsfigs/expansionpath.png}
...
\pdfnote{The total 'rise over run' for the TC curve at a given quantity as the firm optimally chooses its inputs along the expansion path}
...
\includegraphics[height=4.5in]{picsfigs/mesTCcurve.png}

***

\includegraphics[height=4.5in]{picsfigs/mescostcurves}
...
\pdfnote{We are not highlighting this distinction not because it is uninteresting, \textCR
 but because we are time-constrained in this module}
...
\pdfnote{Antitrust authorities want to know if firms are colluding to 'overcharge' \textCR
 regulation, what price to ask natural monopoly to charge \textCR
Macro/growth  aggregate production functions, importance of inputs e.g., human-K \textCR
Impact of trade deals on the economy; who wins/loses \textCR
Effect of min wages and labour laws on prices/competitiveness \textCR
Business strategy: know  own, competitors', supplier's production functions to compete/react to/anticipate changes market}
...
\pdfnote{Some (IO) economists try to 'back out' cost functions from estimated demand functions and pricing}
...
<!---
\includegraphics[height=2in]{picsfigs/csgraphforqn.png}
**Refer to the graph above.  When the price is P1, consumer surplus is** \
- a. A \
- b. A + B \
- c. A + B + D \
- d. A + B + C
\pdfnote{Note, on exam I could ask you to 'select all areas that constitute the consumer surplus'}
-->
<!---
...
- e. only I \
***
\textcolor{gray}{This question relates to textbook application 7.3}
-->
...
\pdfnote{Ans: a}
\pdfnote{On exam, I could ask 'choose all factors likely to favour ALLOWING the merger}
-->
...
\pdfnote{Adv: This spawned a large literature on the 'theory of the firm' \textCR
emphasizing things like incomplete contracts, hold-up, relationship specific assets, \textCR
lobbying costs, and more.}
...
\textcolor{gray}{...standard conditions for 'unique maximum'}
...
\pdfnote{This is also used as a measure of the extent of a firm's market power}
...
[comment]: <> (101BB)
\textcolor{gray}{You should have all seen this before in previous study, so we will quickly outline:}
...
\pdfnote{Adv: There is a sense in which this is resembles game theoretic Nash equilibrium. \textCR
This price leads firms to produce quantities such that they can sell all their output; they don't want to 'deviate' to producing more or less. \textCR
...
\pdfnote{Difficult question: when is is reasonable to do this, and why?}
...
\textcolor{gray}{(given the allocation of private consumption)}

\

... with $G^\ast$  units of PG, *social* MB (sum individual MB) of public good equals MC

\pdfnote{Note that here MC(G) essentially expresses the MRT and MB_i is essentially i's MRS}
...
\pdfnote{Can be derived by solving the  "Pareto problem": max 1 persons utility \textCR
s.t. holding all others' utility at or above a threshold \textCR
and s.t. the "aggregate production constraint" \textCR
See http://community.dur.ac.uk/t.i.renstrom/teaching/PUBECON/Lect_12.pdf for a 2-household case}
...
- Definition of a Public Good, what fits into this category?

-  Why a competitive market will usually *undersupply* a public good (but may still supply *some* of it)
...
-  The difficulties *government* may face in providing the right amount of a public good
...
- Time permitting: brief intro to monopoly (leading to another market failure)
...
\textcolor{gray}{Also see the 'variance' and other 'moments'.}
...
- \textcolor{gray}{Diagonal line btwn $u(x_{low})$ \& $u(x_{high})$; EU in-btwn these}
    - \textcolor{gray}{Horizontal distance $p$ of the way btwn $x_{low}$ \& $x_{high}$ $\rightarrow$ $E(X)$}
...
\textcolor{gray}{...advanced}

\underline{Local measures of risk-aversion}

- Coefficient of *absolute* risk aversion: $A(m) \equiv -\frac{u''(m)}{u'(m)}$

\pdfnote{Note this is preserved under an affine transformation; the value itself may be constant, decreasing, increasing in income...}
...
\pdfnote{These ideas are also relevant to intertemporal substitution and to social welfare functions}

\pdfnote{RRA (ARA) constant--> constant fraction of portfolio (number of dollars) held in risky asset as wealth increases.}
...
- \textcolor{gray}{Continuous action space}
...
\pdfnote{Beem101: We *will* cover continuous actions, but not incomplete information at this point}
...
If you both confess you will each get 3 years. \textCR
}
...
\pdfnote{Read PD in Extensive form; we'll come back to this later}
...
\includegraphics[height=2.2in]{picsfigs/pd_extensive.png}
...
\pdfnote{Order only matters if you observe earlier action}

\small{Typical: extensive form for sequential games, normal form for simultaneous games}


\pdfnote{This fancy terminology seems confusing and unnecessary. \textCR
The reason for it involves more formal definitions of 'extensive' and 'normal' games and some deeper technical issues that we skip here.}

***

###Japanese form

\includegraphics[height=4.2in]{picsfigs/PD_japan.png}


\pdfnote{Japanese form}
...
\pdfnote{The BR is a function of the others' strategies *S*; it may take a different value for each strategy the others play.}
...
\pdfnote{Dominated strategy, ISD  not in text!}
...
\pdfnote{What about 'players never play dominated strategies'?}
...
3. Multiple equilibrium and refinements
...
\textcolor{gray}{Market equilibrium (review): given the equilibrium price \& quantity, no market participant has an incentive to change her behaviour.}
...
No $\rightarrow$ it's (outcome of) a NE.
...
\includegraphics[height=1.5in]{picsfigs/bosmatrix.png}
...
\includegraphics[height=5in]{picsfigs/bosbr1.png}

\pdfnote{This plots the value of w that best responds to h; \textCR
 the probability the wife goes Ballet given the probability the husband goes Boxing.}
...
\includegraphics[height=5in]{picsfigs/bosbr2.png}
...
\pdfnote{Adv: Some texts and authors make this point about 'unpredictability' but others disagree (including myself).
...
$$U_{wife} = \frac{2}{3}$$
\bigskip
\textcolor{gray}{Wife's payoff inferred from symmetry of problem or similarly calculated}
-->
...
[LINK](https://www.youtube.com/watch?v=OvsNgQS_Qa8)

- How do you catch a cloud and pin it down?
    - How do choose among the multiple-equilibria?
    - A flibbertjibbet! A will-o'-the wisp! A clown!

\pdfnote{See <https://youtu.be/M1HwVmY28Pk?t=168> for more information}
...
\includegraphics[height=4in]{picsfigs/BOS_inlecture.png}


\pdfnote{LC: One is assigned the Husband role, the other the Wife role. Each to write down their move in private. \textCR
...
\includegraphics[height=3.5in]{picsfigs/coordgame_inlecture.png}
...
\pdfnote{As you can see from mock exams etc., GT with continuous actions IS something I may test you on!}


***

\textcolor{gray}{With continuous actions...}
...
\pdfnote{Here the iterated dominance or 'rationalizability' is harder to compute; we won't cover this}
...
\pdfnote{BEEM101: covered in tutorial}
...
\includegraphics[height=2.35in]{picsfigs/brsheep.png}
...
\pdfnote{This is the 'continuous action' analogue of the Prisoner's dilemma}


\pdfnote{Both the PD and the above 'tragedy of the commons' game illustrate a similar point: \textCR
...
\includegraphics[height=2in]{picsfigs/moroccansheep.jpg}
...
The lecturer was good at explaining things

Tutorial staff were good at explaining things. (this refers to Stephane and myself)

The content of the module is intellectually stimulating

The module was well organised and ran smoothly

Method(s) of assessment are appropriate for the intended learning outcomes for this module

Feedback received has clarified how I can improve in future

It's clear how student feedback is valued in this module.

***

\footnotesize

I had access to good learning resources for the module (library, reading lists, online, problem sets and solutions etc)

I was encouraged to learn independently in this module.

I have received sufficient advice and support with my studies in the module

The module challenged me to achieve my best work and develop my own intellectual interests.

The module has provided opportunities to apply my learning

Teaching approach and learning opportunities for this module aligned with the intended learning outcomes
...
Module questionnaire: access via the iExeter app and ELE. Open from 4th to the 22nd December.
...
\pdfnote{See 'more formal moral hazard notes' if interested}
...
- NS: Ch 17, plus possible supplements
- T1:10
...
[comment]: <> (2024BB)
###Behavioural Economics: Lesson plan 1
[comment]: <> (2024EE)

[comment]: <> (101BB)
###Behavioural Economics: First hour
[comment]: <> (101EE)

*(Goals of lecture and self-study)*
...
1. Impatience, time-inconsistency, 'hyperbolic discounting' (time permitting)
...
Make your choices on Turningpoint/Responseware :
...
$\rightarrow$  Gamble A: the 1\% chance of 0 is treated as larger than it is?
...
*But* $A$ is the SAME as $C$: 50\% chances of \pounds1000 or \pounds2000,
...
that we wouldn't make if we saw it as \textcolor{green}{'increasing gains'}
...
## Limited willpower and 'hyperbolic discounting' (read at home)
...
Hyperbolic discounting (simple 'Beta-delta' version)
...
Examples ... compare valuation of 5 now (year 0), 10 in 1 period (year 1), 11 in year 2

...if evaluating these in year 0, 1, or 2.
...
Ant weights payoffs at 1 today \& $w=1/2$ on any future day
...
[comment]: <> (2024BB)
###Behavioural Economics: Lesson plan 2
[comment]: <> (2024EE)

###Behavioural economics -- second hour
...
\includegraphics[height=2.7in]{picsfigs/other_regard_IC.png}
...
\begin{centering}
\footnotesize E.g., in ultimatum game, suppose 'real' payoffs imply each player gains 1 util for each pound earned, but loses a 1 util for each pound of difference between payoffs.
\end{centering}
...
\pdfnote{These are basic game theoretic predictions, not Behavioral.}
...
**Results:** In this field experiment men committed more in the Before case; for women, the treatments have no strong effect. However, this may have been a statistical artefact.

\includegraphics[height=3in]{picsfigs/BarGrDonHiByCertainFemaleExperiment.png}


\pdfnote{Treatment effects are significant for men in a variety of statistical analyses, as are gender differences in the impact of the treatment.}

\pdfnote{Other field experiments, lab work: significant difference overall, no strong gender effects.}
