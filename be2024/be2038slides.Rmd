...
Building revealjs slides
- Bulk edits with :source /Users/yosemite/githubs/dr_text_md_tools/md_tex_to_rmd_revealjs.vim
- split into sections, hand-edit
- second-level (vertical) for 'slides if time permits' etc
- try to host and link all of these

Embedding videos... how to 'embed' pages
Incorporate (or link) quizzes, exercises, in-lecture feedback
...
<!---
`r format_with_col("\small{Guided Independent Study per half: 143 hours to be divided between background reading (e.g. 40 hrs), solving/reviewing problem sets (e.g. 40 hrs) and exam revision (e.g. 43 hrs)","blue")`}
-->
...
<big>The 'HANDOUTS' file/web-book<big>
...
`r format_with_col("More challenging questions: see handout","gray")`
...
```{r  fig.cap = '', out.width='90%', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics("guyfawkes.jpeg")
```
...
<aside class="notes">read it if you are interested and if it helps you understand the concepts below.</aside>
...
`r format_with_col("Very similar to the consumer optimisation we discussed","gray")`
...
`r format_with_col("Consider: why/when would we expect DRS/IRS/CRS?","blue")`
...
<aside class="notes">This is not covered at this point in the NS text.
...
*Note: many of the NS text 'problems' here are a bit more math intensive; we will focus on conceptual problems*
...
`r format_with_col("Posted on VLE. The question 'not-16.7' is NOT the NS text's q 16.7. They got it wrong","red")`




See VLE 'big file'. Note that the question 'not-16.7' is NOT the NS text's q 16.7. They got it wrong}
...
- `r format_with_col("Diagonal line btwn $u(x_{low","gray")`)$ & $u(x_{high})$; EU in-btwn these}
    - `r format_with_col("Horizontal distance $p$ of the way btwn $x_{low","gray")`$ & $x_{high}$ $\rightarrow$ $E(X)$}
...
`r format_with_col("no focus on mixed strategies","gray")`
...
- `r format_with_col("Not covered/examined: Continuous action space","gray")`

<aside class="notes">Although this might better help you understand the public goods interactions; so recommended reading</aside>
...
<aside class="notes">Intended Learning Outcomes</aside>
<aside class="notes">Many of you have seen game theory before; we will be more precise here, and introduce several concepts you have not seen before</aside>
...
<aside class="notes">What about 'players never play dominated strategies'?</aside>
...
2. Pure and Mixed strategies \footnotesize{(Mixed strategies: get the basic concept, no need to *compute* these)}
...
`r format_with_col("Market equilibrium (review): given the equilibrium price & quantity, no market participant has an incentive to change her behaviour.","gray")`
...
<aside class="notes">Plots the value of w that best responds to h; probability wife goes Ballet given probability husband goes Boxing.</aside>
...
<aside class="notes">Next, plot similar BRF for husband, on other axis...</aside>
...
<aside class="notes">See <https://youtu.be/M1HwVmY28Pk?t=168> for more information</aside>
...
We see a very similar strategic concern in other contexts, including 'which firm enters a market first'</aside>
...
that we wouldn't make if we saw it as `r format_with_col("'increasing gains'","green")`
...
<aside class="notes">These are basic game theoretic predictions, not Behavioral.</aside>
...
```{r  fig.cap = '', out.width='90%', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics("BarGrDonHiByCertainFemaleExperiment.png")
```


<aside class="notes">Treatment effects are significant for men in a variety of statistical analyses, as are gender differences in the impact of the treatment.</aside>

<aside class="notes">Other field experiments, lab work: significant difference overall, no strong gender effects.</aside>
