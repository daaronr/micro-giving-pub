# Utility and Choice [NS Chapter 2; 2 hours]
...
`r format_with_col("We now consider the very very fundamental building blocks behind the (in)famous "neoclassical economics model." Accept these"axioms" nearly everything flows from it!","gray")`
...
```{r  fig.cap = '', out.height='3in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
David Autor's notes (goo.gl/aW3DxB; MIT Microeconomic Theory and Public Policy): [Lecture 3 - Axioms of Consumer Preference and the Theory of
Choice](https://ocw.mit.edu/courses/economics/14-03-microeconomic-theory-and-public-policy-fall-2016/lecture-notes/MIT14_03F16_lec3.pdf)
...
`r format_with_col("The Autor reading refers to Cardinal versus Ordinal utility. We use the latter in demand/consumer theory.","gray")`
...
`r format_with_col("Also 'forbidden' in the classical model: the *time","gray")` or \emph{frame* in which I make the *decisions* must not affect my choices. This will be relaxed when we discuss behavioral economics.}
...
`r format_with_col("A similar idea holds if I am indifferent between one pair of these. If this seems confusing it may be because it is *too","gray")` obvious (although behavioral and experimental economists claim to find violations).*
...
`r format_with_col("Strictly, most results only require 'local nonsatiation'; starting from any consumption point, increasing the amount of *some","RawSienna")` good by a tiny $\epsilon$ amount will yield a preferred point. This will yield results like 'people will spend all of their wealth' and '(roughly) people will never buy a more expensive bundle when a cheaper bundle is preferred.'*
...
`r format_with_col("Comprehension Q: Rank order of preference between points A-E.","blue")` \textcolor{gray}{Note: I've asked this as a MCQ on exams!} (Answers to most comprehension questions are given in footnotes. Try to answer it yourself before looking at the footnote.) ^[Ans: $E \succ A \sim B \sim C \sim D \succ F$]
...
`r format_with_col("Q: How do we know $E \succ B$ ?","blue")`^[A:  It has more soft drink, and the same amount of hamburgers. By Assumption 3: more is preferred to less.]


<br> \bigskip


`r format_with_col("Q:  How do we know $E \succ A$ ?","blue")`^[$B \sim A$ because they are on the same indifference curve
...
`r format_with_col("","RawSienna")` Note that the line between two bundles depicts all 'convex combinations' of these $\alpha$ of D and $1-\alpha$ of A, where $\alpha \in (0,1)$.
...
```{r  fig.cap = '', out.height='2.7in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='2.6in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='2.8in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='2.0in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='1.8in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='3.5in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='2.1in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='3.7in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
```{r  fig.cap = '', out.height='3.6in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
`r format_with_col("","RawSienna")` \textcolor{gray}{We might think that such insurance is provided at its true marginal cost; however there are various reasons why the cost to firms may be above this, including transactions cost, moral hazard, and adverse selection}
...
```{r  fig.cap = '', out.height='1.7in', fig.asp=.4, fig.align='center',  echo = FALSE}
...
`r format_with_col("","RawSienna")` \textcolor{gray}{Some texts and authors make this point about 'unpredictability' but others disagree (including myself). Making sure that it is impossible to predict your later play from your previous is only important if you are playing the same game repeatedly, and being observed.}
...
```{r  fig.cap = '', out.height='1.9in', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics("picsfigs/BOS_sequential.png")
```
...
$$(-2) \times (1 + g + g^2 + g^3 + <br> \bigskip ) \geq (-1) + -3 \times (g + g^2 + g^3 + \bigskip)$$

<br> \bigskip

$$g + g^2 + g^3 + <br> \bigskip \geq 1$$
...
`r format_with_col("","RawSienna")` \textcolor{gray}{The utilities, and decisions, of family members are highly connected. There is an ongoing debate and research programme considering when to model a household's decisions as 'unitary' and when to consider the individual preferences of the household members.}
...
Details:  `r format_with_col("2 rounds over 2 academic years","gray")`. \textcolor{gray}{2 prizes in round 1, Amazon  only in round 2.}  \textcolor{gray}{First covering Faculty of Social Science, then all departments.} \textcolor{gray}{Sign in with email, orthogonal employability treatments.}
