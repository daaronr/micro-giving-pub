...
```{r  fig.cap = 'Famous photograph by Ansel Adams, who loved black and white photography, colorised using De-Oldify by Jason Antic', out.width='50%', fig.asp=.4, fig.align='center',  echo = FALSE}
...
See audio/slides here, from 101:25 - https://recapexeter.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=9B44DE1E-2569-4076-9D7B-AACC0064717D#
...
<div class="marginnote">
DR #TODO: I intend to add a video here discussing the below 'motivating exercise'.
</div>
...
O-R first consider a 'value function' "$v$ that attaches to each alternative a number" ... the preference relation here "is defined by $x \succsim y$  if and only if $v(x) \succsim v(y)$. 
...
<div class="marginnote">
DR #TODO: I intend to add a video or audio content discussing the above.
</div>
...
So if utility is (at best) defined as 'the thing that people maximise', does it have any meaning, or does it simply lead to circular reasoning. What do you think? (My thoughts in the fold).
...
Empirically, it is hard to imagine how these could be measured: if utility is only indirectly and partially observable through an individual's choices... when do we observe choices that trade off between one person and another in ways that reflect the 'sum of their utilities'? (Never, I guess.)
...
```{block2,  type='technote'}
There are 'important' economic theorems specifying 'sufficient conditions for preferences to be represented by a utility fncn' ; we will not cover it, but unfold below for some more discussion, if you are interested.
...
`r format_with_col("See O-R page 13, figure 1.4, for a depiction of  lex. preferences withinin a square.", "green")`
...
This is 'dense'. In more plain English, a 'choice problem' is a subset of alternatives (perhaps those alternatives which are 'feasible' for the individual to choose among).
...
<div class="marginnote">
$A$ This may be the set of all alternatives Ratchell can afford to buy given his wealth, or the set of combination of goods and liesure time he can consume given his productivity and the prices he faces in the market, etc. 
</div>
Utility
...
This comes from [@luceTheoryIndividualChoice1957], as cited by O-R.
...
Previously descrived utility functions: predict choices under certainty. They made no predictions for choices with uncertain probabilistic outcomes!
...
<div class="marginnote">
Todo: look up and include some graphics showing the pdf and cdf, or create these within R.
...
Expected Utility is the 'most commonly assumed' framework in economic theory. We will consider certain “axioms” that seem reasonable, particularly the ‘independence’ axiom. We can formally prove people’s preferences obey these axioms, then their preferences must be "consistent with expected utility".
...
...
is sometimes called "normative Economics".  But this is confusing as "normative" is also used to refer to several other things.
...
<div class="marginnote">
While I descrive a set of countable alternatives, O-R define the below over the more general idea of a 'set of lotteries $L(Z)$ over prizes $Z$'.
...
Remember, each lottery $p$ is a set of probabilities over each possible alternative. With a finite set of $n$ alternatives $z_1, z_2, ...,z_n$ we have probabilities $p_1, p_2, ... p_n$ over eeach.
...
Where $v(\cdot)$ is a 'value function' (aka 'sub-utility function', aka 'Bernoully function').
...
.... it implies that the expected utility (the probability-weighted sum of the $v$ terms) is of a  gamble is always higher when the expected monetary value of this gamble is higher?
...
To be added or mentioned here:
  
O-R formal definition of risk aversion
  
```

### Risk preferences and EU {-#eu-risk}

Imagine the realised utility $v(y)$ that arises from the consumption chosen with wealth $y$.
...
(Content to be added here, drawing both from O-R and from my previous notes)
...
<div class="warning">
These are not the same as the 'complements' and 'substitutes' in demand functions which we may see you later, referring to the impact of changing prices.
</div>
...
> Continuity is a property of a consumer’s preference relation that captures the idea that if a bundle x is preferred to a bundle y then bundles close to x are preferred to bundles close to y.

<div class="marginnote">
You may skip the formal definition of continuity in O-R, and  also skip the proof of  it's relationship to a continuous utility function. 

However, knowing the definition of continuity as a mathematical property may be relevant in your future studies and research..
</div>
...
```{r block05, fig.cap = 'Convex preferences - from McDL', out.width='40%', fig.asp=.4, fig.align='center', echo = FALSE}
...
> A preference relation on  $R^2_+$ that is represented by a concave [utility] function is convex

It will also have convex indifference curves.

Make sure you don't get this confused.

```  

> The convexity of a strongly monotone preference relation is connected with the property known as decreasing marginal rate of substitution.  

#### More formal concept: [quasiconcavity (optional, linked as supplement)](#quasi-conc)   {-}

This is the 'preferred assumption' which resembles convexity, but has several advantages as a concept. For example, it doesn't require differentiable preferences, and its properties are not affected by monotonic transformations of utility functions. This is the concept that you might be taught in a PhD module. I think it may also have some value in understanding optimization problems in general, which is an important part of research and industry and finance, I believe.  [Linked as supplement here](#quasi-conc).
...
However,  do have a look at O-R figure 4.5  to get a sense of what differentiability  means for indifference curves (indifferencesets).
...
For each unit of $x_2$ I give up I save $p_2$, so the more costly is $x_2$ the more I can save by giving up 1 unit of it,
thus, the less I need to sacrifice of $x_2$ to get another unit of $x_1$.
...
We could use this to demonstrate "gains to trade". Suppose two people with the same preferences, each with a distinct bundle (at a point other than XXX). They can divide up these bundles among each other, each taking a convex combination of the bundles, and the other taking the remainder, which is also a convex combination.
...
There are another type of demand functions  that have very interesting properties for theory and welfare analysis called "Hicksian demand functions" or "compensated demand functions".  These take prices and *utility* as the arguments.  We will not cover these in 2020  because of time constraints.
...
We can demonstrate that you will choose point A, yielding utility $100$.

What is special about point A?
...
At this point (but not in general!) we have that  "Slope of budget constraint = slope of indifference curve". 
...
```{r  fig.cap = '', out.height='5%', fig.asp=.4, fig.align='center',  echo = FALSE}
 knitr::include_graphics(file.path("picsfigs","glasses.png"))
```
...
Setting it equal to zero and rearranging it yields the rate, at the margin, one is willing to give up $x_2$ for $x_1$:
...
Thus if everyone has these preferences impact of 1 consumer's income increasing by £1 million is the same as 1 million consumers gaining £1. This (possibly dubious) assumption underlies 'representative consumer' models.
...
<div class="marginnote">
This relates to the puzzle of 'why should monetary policy and inflation have any real effect on the econom?'
</div>
...
Q: What happens to the quantity purchased of some good as an individual's income increases?","blue")`
...
To cover this in more detail, we would need to define another concept called "Hicksian  demand". 
...
To consider constrained optimization (with many goods) using the Lagrangian method  see [THIS supplement](#lagrange) (based on David Autor's notes)

### Some doctoral-prep concepts {-#docprep}
...
- Substitution and income effects, and the Slutsky decomposition
...
- Q1a: utility function 'distance from origin'; depict, determine monotonicity, continuity, convexity

- Q2 

- Q4a: Quasilinear preferences; depict indifference curves
...
- problem 5 (introduces the 'expenditure function')
...
Audio/slide presentations (unfold) 
...
<iframe src="https://recapexeter.cloud.panopto.eu/Panopto/Pages/Embed.aspx?id=9b44de1e-2569-4076-9d7b-aacc0064717d&autoplay=false&offerviewer=true&showtitle=true&showbrand=false&start=0&interactivity=all" height="405" width="720" style="border: 1px solid #464646;" allowfullscreen allow="autoplay"></iframe>                              
