...
Yes I did. Third degree price discrimination, or 'market segmentation' is a much simpler concept than second-degree price discrimination, which involves self-selection. We will return to the latter [below](#2dpd).
...
## 'Second-degree price discrimination', i.e., 'self-selection', i.e., 'implicit discrimination' {#2dpd}
...
### Formal treatment (O-R) {#2dpd_formal} 

The self-selection problem is characterized by a 'menu' of items that the firm offers, each item having a price and a quantity (or level of quality). To achieve the desired "self selection", as implied above, several 'constraints' must be met. These constraints Are necessary so that each of the consumers wants to choose the bundle 'intended for them', and also wants to buy something rather than nothing. 
...
> A collection $(V^i)_{i=1)^n$ of increasing continuous [value] functions, where $V_i : [0,1] \rightarrow  R^+$ and $V_i(0) = 0.$ 

To consider this most simply, they 'normalise' the units of 'quantity of the good' (or 'quality of the good') to fall between 0 and 1, including the endpoints. They normalise the value of consuming 0 units to 0. 

<div class="marginnote">
I keep mentioning that this can also express units of quality, as the problem is mathematically equivalent if, as is commonly seen in the real world, a firm uses product quality to get consumers to self-select.
</div>
...
> If $V_i(q)−m ≥ 0$ for some $(q,m) \in M$, consumer $i$ chooses an option $(q,m) \in M$ for which $V_i(q)−m$ is maximal; otherwise she buys nothing. The producer chooses M so that the consumers’ choices maximize her profit.
...
- *'Participation constraint'*: Each consumer must prefer to choose the menu item the firm wants it to choose over "not participating at all", i.e., not buying anything. 
...
To find the most profitable strategy, we need to consider all of the possible implementations, determine the profits from each, and find the one that maximizes these profits.


\

<!-- add here: a more concrete parametric 2-consumer case -->
...
```
We know that the optimal price for society is where $P = MC$ which is £19. We can see from the above graph that this is the peak of the total surplus curve, as the theory predicts.
