...
<small> Note: Uncertain choices $\rightarrow$ 'Von Neumann - Morgenstern' utility functions;  somewhat more restrictive, thus better identified. </small>
...
<small> `r format_with_col("If the product is a 'bad' (e.g., pollution), redefine as the *absence* of the product", "grey")` </small>
...
![Fig 2.5](picsfigs/particularpreferences.png){width=65%} \
...
![Utility max depiction](picsfigs/utilmax.png){width=70%} \
...
<small> This yields the above optimisation (first-order) conditions (subject to non-negativity constraints) </small>
...
I.e., 
$$\nabla U(\mathbf{x^\ast})\cdot \mathbf{p}=0$$


<aside class="notes">

The dot product of  the 'gradient vector' of the partial derivatives of utility wrt each element of $x$, evaluated at the optimum and the price vector must equal zero. 

Which, using matrix algebra, tell us that, at optimum 'the rate of increase in utility is greatest in the direction of the vector tangent to the budget line (plane)'.. 
Which tells us that the plane tangent to the 'indifference set' at this point is the budget plane. 
...
<small> where a, b are positive constants </small>
...
<small> More generally (and usefully) $U=x_1+f(x_2,...,x_n)$ linear in one 'numeraire' good $x_1$. </small>
...
##  Second problem set: Coming soon

More on preferences, Utility, Consumer optimization,  individual and market demand curves.
